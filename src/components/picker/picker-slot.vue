<template>
    <div class="wk-picker-slot" :class="{'wk-picker-slot-divider': divider}">
        <div v-if="!divider" class="wk-picker-slot-wrapper" :style="{'height': contentHeight + 'px'}" ref="wrapper">
            <div class="wk-picker-item" :class="{'selected': item === value}"
                 v-for="item in values">{{item.text || item}}
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        watch: {
            ...
            value () {
                this.doOnValueChange()
                this.$emit('slot-select', this.id, this.value,)
            }
        },
        components: {}
    }
